...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const sumOfSquares = (x2 - x1) ** 2 + (y2 - y1) ** 2;
...
function getSumOfDigits(num) {
  ...
  const stringToCharacters = numToString.split('');
  const arrayDigits = stringToCharacters.map((character) => Number(character));
  let sum = 0;
...
function toFixed(number, fractionDigits) {
  ...
  const stringNumber = String(result);
  return stringNumber;
}
